...
const europeRecipes = JSON.parse(fs.readFileSync('/workspace/data/recipes/europe/europe_all.json', 'utf-8'));

// Mapeo de países a placeId (necesitaremos crear estos places después)
const countryToPlaceId = {
  'Italy': 'it-',
  'France': 'fr-',
  'Spain': 'es-',
  'Germany': 'de-',
...
const createPlaceId = (country, region, city) => {
...
const convertedRecipes = europeRecipes.map((recipe, index) => {
  ...
  return {
    ...
    ingredients: recipe.ingredients.map(ing => ({ text: ing })),
    steps: recipe.instructions,
    ratingAvg: parseFloat((4.5 + Math.random() * 0.4).toFixed(1)),
...
convertedRecipes.forEach((recipe, index) => {
  output += `  {\n`;
  output += `    id: "${recipe.id}",\n`;
  ...
  output += `    ],\n`;
  output += `    ratingAvg: ${recipe.ratingAvg},\n`;
...
// Escribir el archivo
fs.writeFileSync('/workspace/atlas-gastronomico/src/data/recipes-europe.ts', output);
...
const countries = [...new Set(europeRecipes.map(r => r.country))];
console.log(`\n🌍 Países incluidos: ${countries.join(', ')}`);

// Mostrar ejemplos de recetas
console.log(`\n📋 Primeras 3 recetas:`);
convertedRecipes.slice(0, 3).forEach(r => {
  console.log(`   - ${r.dishName} (${r.placeId})`);
});
